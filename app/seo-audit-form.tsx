...
export default function SeoAuditForm() {
  ...
  return (<>
    ...
    {report && <section className="reportBox hpv31FormCard" style={{ marginTop: 24 }}><h3>AI SEO 诊断报告（中文）</h3><p><b>总分：</b>{report.score} / 100 ｜ <b>风险等级：</b>{report.riskLevel}</p><p><b>核心问题：</b>{report.issueCount} 项（高优先级 {report.highPriorityCount} 项）</p><p>{report.executiveSummary}</p>{report.sections.map((s) => <div key={s.title}><h4>{s.title}</h4><p>{s.summary}</p><ul>{s.items.map((i) => <li key={i}>{i}</li>)}</ul></div>)}</section>}
  </>);
}
