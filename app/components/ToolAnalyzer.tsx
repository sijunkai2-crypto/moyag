...
export default function ToolAnalyzer({ toolType }: { toolType: string }) {
  ...
  const run = async () => {
    ...
    try {
      const resp = await fetch(`/api/tools/${toolType}`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(form) });
  ...
  return <div className="hpv32FormCard"><div className="auditForm hpv33ToolForm">{fields[toolType].map((f)=><label key={f.name}>{f.label}<input required={f.required} value={form[f.name] || ''} onChange={(e)=>setForm((s)=>({ ...s, [f.name]: e.target.value }))} placeholder={f.required ? 'https://example.com' : 'Optional'} /></label>)}<button className="submitBtn" type="button" onClick={run} disabled={loading}>{loading ? 'Analyzing...' : 'Run Analysis'}</button>{error ? <p className="formMessage error">{error}</p> : null}</div>{result ? <ToolResultPanel result={result} /> : null}</div>;
}
